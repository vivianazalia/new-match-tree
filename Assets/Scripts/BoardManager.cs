...
public class BoardManager : MonoBehaviour
{
    ...
    private void CreateBoard(Vector2 tileSize)
    {
        ...
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                ...
                //agar saat random pertama tidak ada tile dengan warna sama
    ...
    //Id yang tersedia untuk setiap tile
    ...
    Vector2Int GetTileIndex(TileController tile)
    {
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
    ...
    public List<TileController> GetAllMatch()
    {
        ...
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                List<TileController> tileMatched = tiles[x, y].GetAllMatch();
